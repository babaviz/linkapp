/**
 * PropertyChatScreen - Real-time chat with property owners using Stream Chat
...
type PropertyChatNavigationProp = StackNavigationProp<PropertyStackParamList, 'PropertyContact'>;
...
export default function PropertyChatScreen() {
  const navigation = useNavigation<PropertyChatNavigationProp>();
  const { handleBack } = useBackNavigation({ fallbackScreen: 'PropertyHub' });
  const route = useRoute();
  const { user } = useAppSelector((state) => state.auth);
  const { isConnecting, connectionError, getChannel, createPropertyChannel } = useStreamChat();
  const { isReady: isChatClientReady } = useStreamChatClient();
  
  const { property, conversationId: existingConversationId } = (route.params as RouteParams);
  ...
  const [ownerName, setOwnerName] = useState<string>(property?.title || '');
  ...
  const routeParams = useMemo(() => ({ property, existingConversationId }), [property?.id, existingConversationId]);
  
  // Keep ownerName in sync when navigating between chats
  useEffect(() => {
    setOwnerName(property?.title || '');
  }, [property?.id, property?.title]);

  // Reset local state when navigating between different listing chats.
  useEffect(() => {
    setChannel(cachedChannel);
    setError(null);
    setIsLoading(!cachedChannel);
    isInitializingChatRef.current = false;
  }, [cachedChannel, property?.id, existingConversationId]);
  ...
  const initializeChat = useCallback(async () => {
    ...
    if (user.id === property.owner_id) {
      setError('You cannot message yourself. View inquiries in the Inquiries screen.');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      let propertyChannel: Channel | null = null;
      if (existingConversationId) {
        propertyChannel = streamChatService.getCachedChannelByCid(existingConversationId);
      }

      if (!propertyChannel && existingConversationId) {
        const parts = existingConversationId.split(':');
        if (parts.length === 2 && parts[0] && parts[1]) {
          propertyChannel = await getChannel(parts[0] as any, parts[1]);
        }
      }
      ...
      if (!propertyChannel) {
        const propertyLocationLabel =
          typeof property.location === 'string'
            ? property.location
            : [property.location?.town, property.location?.county].filter(Boolean).join(', ');

        propertyChannel = await createPropertyChannel(property.id, property.owner_id, {
          title: property.title || `Property Chat - ${property.location || 'Location'}`,
          price: (property as any).price ?? 0,
          location: propertyLocationLabel || '',
          image: typeof property.images?.[0] === 'string' ? property.images[0] : undefined,
        });
      }
      ...
      setChannel(propertyChannel);

      // Derive proper display name from channel data or members
      const channelData = propertyChannel.data as Record<string, unknown> | undefined;
      const members = Object.values(propertyChannel.state.members);
      const otherMember = members.find(m => m.user_id !== user.id);
      const derivedName = otherMember?.user?.name || 
                          channelData?.property_title as string || 
                          property.title || 
                          'Property Owner';
      if (derivedName) {
        setOwnerName(derivedName);
      }

      // Send initial message if this is a new conversation
      if (!existingConversationId) {
        const cid = propertyChannel.cid;
        if (cid && !sentInitialMessageCidsRef.current.has(cid)) {
          sentInitialMessageCidsRef.current.add(cid);
          InteractionManager.runAfterInteractions(() => {
            if (propertyChannel.state.messages.length === 0) {
              propertyChannel
                .sendMessage({
                  text: `Hi! I'm interested in your property "${property.title}". Could you please provide more details?`,
                })
                .catch(() => {});
            }
          });
  ...
  const handleChannelInfoPress = useCallback(() => {
    ...
    Alert.alert(
      'Property Chat',
      `Property: ${channelData?.property_title || 'Unknown'}\nLocation: ${channelData?.property_location || 'Unknown'}`,
